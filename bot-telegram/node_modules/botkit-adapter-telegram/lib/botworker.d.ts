...
/**
 * This is a specialized version of [Botkit's core BotWorker class](core.md#BotWorker) that includes additional methods for interacting with Telegram.
 * It includes all functionality from the base class, as well as the extension methods below.
 *
 * When using the TelegramAdapter with Botkit, all `bot` objects passed to handler functions will include these extensions.
 */
export declare class TelegramBotWorker extends BotWorker {
    /**
     * A copy of the TelegramAPI client giving access to `let res = await bot.api.callAPI(path, method, parameters);`
     */
    api: TelegramAPI;
    /**
     ...
     *
     * When used with the TelegramAdapter's multi-tenancy mode, it is possible to spawn a bot instance by passing in the Telegram page ID representing the appropriate bot identity.
     ...
     * ```
     * @todo
     * Check weather this is still relevant for Telegram. Right now no harm is done if this is not change. But we may
     * need to update it depending on further analysis
     *
     * @param botkit The Botkit controller object responsible for spawning this bot worker.
     * @param config Normally, a DialogContext object.  Can also be the ID of a Telegram page managed by this app.
     */
